...
async function getData(): Promise<StationRow[]> {
  ...
  const rows: StationRow[] = stationStatusData.data.stations.filter(s => s.is_installed && s.is_renting).map(s => {
    const info = stationInformationData.data.stations.find(si => si.station_id === s.station_id);
